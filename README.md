...
| Your chat width resets to a narrower value on its own | Update to **0.21.11+** — the saved width was being clamped against the *windowed* browser viewport when the page loaded, and the shrunken number was then written back to storage by the next settings change. Opening Kick once in a narrow window permanently shrank a width set on a wide display. The load-time clamp is now the 260–640px bounds only. A width already shrunk by an earlier version needs setting once more. |
