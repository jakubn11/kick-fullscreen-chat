...
| Stop moving the mouse for the hide delay (2–8s, default 4s) | The control cluster and stream-info overlay fade out in sync with Kick's controls/timeline. Any mouse movement brings them back instantly. While the cluster is fading back in (~0.2s) it stays click-through, so a click aimed at the player isn't swallowed by a button that is still invisible. |
| Switch to another app or tab and click back into the browser | The controls stop accepting clicks and keypresses the moment the window is left, and start again 350ms after focus returns, so the click that re-activates the window can't land on whichever control the cursor was parked over however the browser orders the two events. Leaving also drops keyboard focus from any control that held it, so the reflexive Space/Enter to pause the stream goes to the player rather than to a button the browser refocused. |
...
| The settings panel opens on its own, or a setting changes by itself (often after switching back to the browser) | Update to **0.21.5+** — the controls ignore clicks and keypresses for a moment after the window or tab regains focus, so the click that re-activates the browser can't land on the gear (or on a panel switch). 0.21.3 introduced that guard but started it only once the browser told the page focus was back, and the re-activating click sometimes arrived first; 0.21.5 arms it as the window is *left*, so the ordering no longer matters. Earlier fixes cover the other causes: 0.21.1 stopped the faded-out cluster from being clickable while still invisible, and 0.20.1/0.21.1 made the controls drop keyboard focus after a mouse click so Space and the arrow keys go to the video. |
