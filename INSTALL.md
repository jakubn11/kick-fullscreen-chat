...
| Switch to another app or tab and click back into the browser | The controls ignore clicks and keypresses for 350ms after focus returns, so the click that re-activates the window can't land on whichever control the cursor was parked over — and the reflexive Space/Enter to pause the stream goes to the player, not to a button the browser refocused. |
...
| The settings panel opens on its own, or a setting changes by itself (often after switching back to the browser) | Update to **0.21.3+** — the controls now ignore clicks and keypresses for a moment after the window or tab regains focus, so the click that re-activates the browser can't land on the gear (or on a panel switch) even when the cluster never faded away. Earlier fixes cover the other two causes: 0.21.1 stopped the faded-out cluster from being clickable while still invisible, and 0.20.1/0.21.1 made the controls drop keyboard focus after a mouse click so Space and the arrow keys go to the video. |
